...
import { trocarCodigoPorToken } from '../lib/ml'
import { atualizarTokensML, getClienteAtivo } from '../lib/storage'
import { CheckCircle, XCircle, Loader } from 'lucide-react'

export default function AuthMLCallback() {
  ...
  useEffect(() => {
    ...
    trocarCodigoPorToken(code)
      .then(tokens => {
        atualizarTokensML(cliente.id, tokens)
        setStatus('ok')
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F4F6F9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center', background: '#fff', borderRadius: 16, padding: '40px 48px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
        {status === 'loading' && (
          <>
            <Loader size={40} color="#F6C90E" style={{ animation: 'spin 1s linear infinite', marginBottom: 16 }} />
            <p style={{ fontWeight: 700, color: '#1A202C', fontSize: 16 }}>Conectando Mercado Livre...</p>
        ...
        {status === 'ok' && (
          <>
            ...
            <p style={{ fontWeight: 800, color: '#1A202C', fontSize: 18 }}>Mercado Livre conectado!</p>
